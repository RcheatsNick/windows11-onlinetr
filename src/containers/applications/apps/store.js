          ...
          <div className="storeSection w-full absolute top-0">
            ...
            <div id="apprib" className="frontCont amzApps my-8 py-20 w-auto mx-8 flex justify-between noscroll overflow-x-scroll overflow-y-hidden">
              <div className="flex w-64 flex-col text-gray-100 h-full px-8">
                <div className="text-xl">Windows Uygulamaları</div>
                <div className="text-xs mt-2">Bu olmazsa olmaz uygulamalarla Windows deneyiminizi yeni zirvelere taşıyın</div>
              </div>
                ...
                {apprib && apprib.map(x=>{
                  ...
                  return (
                    <div className="ribcont rounded my-auto p-2 pb-2">
                      ...
                      <div className="text-xss mt-8">{x.charCodeAt(4)%2?"Ücretsiz":"Yüklü"}</div>
                    </div>
                  )
                })}
              </div>
            ...
            <div id="gamerib" className="frontCont amzGames my-8 py-20 w-auto mx-8 flex justify-between noscroll overflow-x-scroll overflow-y-hidden">
              <div className="flex w-64 flex-col text-gray-100 h-full px-8">
                <div className="text-xl">Öne Çıkan Oyunlar</div>
                <div className="text-xs mt-2">Xbox oyunları oynamak ve yeni bir favori bulmak için eğlenceyi keşfedin</div>
              </div>
              <div className="flex w-max pr-8">
            ...
            <div id="movrib" className="frontCont amzMovies my-8 py-20 w-auto mx-8 flex justify-between noscroll overflow-x-scroll overflow-y-hidden">
              <div className="flex w-64 flex-col text-gray-100 h-full px-8">
                <div className="text-xl">Öne Çıkan Filmler</div>
                <div className="text-xs mt-2">En son hit filmleri kiralayın veya satın alın ve bunları evde veya hareket halindeyken izleyin</div>
              </div>
              <div className="flex w-max pr-8">
                {movrib && movrib.map(x=>{
                  var stars = 3 + (x.charCodeAt(0)+x.charCodeAt(1))%3;
                  return (
                    <div className="ribcont rounded my-auto p-2 pb-2">
                      ...
                      <div className="text-xss mt-8">{x.charCodeAt(4)%2?"Ücretsiz":"Alınan"}</div>
                    </div>
                  )
                })}
              </div>
