...
(function(global) {
  ...
  var
    ...
    //name of the days
    dayName = {
      full: ['Pazartesi', 'Salı', 'Çarşamba', 'Perşembe', 'Cuma', 'Cumartesi', 'Pazar'],
      d: ['P', 'S', 'Ç', 'P', 'C', 'C', 'P'],
      dd: ['Pa', 'Sa', 'Ça', 'Pe', 'Cu', 'Cmrt', 'Pzr'],
      ddd: ['Paz', 'Sal', 'Çar', 'Per', 'Cum', 'Cmrts', 'Pazr']
    };
  ...
  function getCalendar(year, month, date) {
    ...
    if (year < START_YEAR || year > END_YEAR) {
      global.console.error("Geçersiz Yıl");
      return false;
    }
    if (month > 11 || month < 0) {
      global.console.error("Geçersiz Ay");
      return false;
    }
    if (date > 31 || date < 1) {
      global.console.error("Geçersiz tarih");
      return false;
    }
  ...
  dycalendar.draw = function(option) {
    ...
    if (typeof option === "undefined") {
      global.console.error("Seçenek eksik");
      return false;
    }
  ...
  function drawCalendar(option) {
    ...
    switch (option.type) {
      ...
      default:
        global.console.error("Geçersiz tür");
        return false;
    }
